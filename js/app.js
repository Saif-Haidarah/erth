class SoundEngine {
  ...
  setState(state) {
    ...
    switch(state) {
      case 'idle': set(this.layers.idle, 0.04); set(this.layers.turbine, 0.0); break;
      case 'core': set(this.layers.idle, 0.06, 0.3); set(this.layers.turbine, 0.08, 0.4); freq(this.layers.turbine, 220, 0.4); break;
      case 'detail': set(this.layers.idle, 0.03, 0.1); set(this.layers.turbine, 0.14, 0.2); freq(this.layers.turbine, 480, 0.2); break;
      case 'stealth': set(this.layers.idle, 0.01); set(this.layers.turbine, 0.0); break;
    }
  }
}
...
function toggleLang() {
  ...
  document.querySelectorAll('[data-en]').forEach(el => { 
    let tmp = el.innerHTML; el.innerHTML = el.getAttribute('data-en'); el.setAttribute('data-en', tmp); 
  }); 
}
...
function loadImg(src, cb) {
  ...
  console.log(`📁 Full URL will be: ${new URL(src, window.location.href).href}`);
  ...
  img.onerror = (e) => { 
    ...
    console.error(`🔍 Check: 1. File exists? 2. Extension matches? 3. Case sensitivity?`);
    loader.classList.remove('active'); 
    showToast('خطأ في تحميل الصورة - راجع Console');
  };
...
function startDissection() {
  console.log('🖱️ Clicked engine. Current layer:', curLayer);
  if (curLayer === 'shell') {
    audio.init(); audio.setState('core');
    curLayer = 'core';
    loadImg(images.core, () => {
...
function drillTo(part) {
  audio.setState('detail'); curLayer = part; overlay.classList.remove('active');
  loadImg(images[part]); btnBack.style.display = 'block';
}

function goBack() {
  audio.setState('core'); curLayer = 'core';
  loadImg(images.core, () => { overlay.classList.add('active'); btnBack.style.display = 'none'; });
}

function resetAll() {
  audio.setState('idle'); curLayer = 'shell';
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('🚀 ERTH App Initialized');
  document.getElementById('langToggle').addEventListener('click', toggleLang);
  engine.addEventListener('click', startDissection);
  
  document.querySelectorAll('.part-btn').forEach(btn => {
    btn.addEventListener('click', (e) => { e.stopPropagation(); drillTo(btn.dataset.part); });
  });
